...
// 做乘法運算時，會自動將 l = '8' 的類型轉換為數字，所以 3*8=24，24 > 21，因此為 true
...
if (playerA === playerB) {
  console.log(`玩家1 出${playerA}，玩家2 出${playerB}，平局`);
} else if (winCondition[playerA] === playerB) {
  console.log(`玩家1 出${playerA}，玩家2 出${playerB}，玩家1 獲勝!`);
} else {
  console.log(`玩家1 出${playerA}，玩家2 出${playerB}，玩家2 獲勝!`);
}
  ...
  coaches: [
    {
      ...
      personIntroduction: '王教練擁有 5 年教學經驗，專精於提升學員的肌力與減脂，適合希望快速達成體能目標的學員',
    ...
    },
    {
      ...
      personIntroduction: '李教練是一位瑜伽大師，擁有 10 年教學經驗，擅長幫助學員雕塑完美體態，適合希望改善姿態與柔軟度的學員',
...
if (performanceData["coaches"][0]["performance"] < 50000) {
  let difference = 50000 - performanceData["coaches"][0]["performance"];
  performanceData["coaches"][0]["performance"] += difference;
}

// 練習：第二位教練（可將下方程式碼註解移除，完成答題）

if (performanceData["coaches"][1]["performance"] < 50000) {
  let difference = 50000 - performanceData["coaches"][1]["performance"];
  performanceData["coaches"][1]["performance"] += difference;
}
...
// 練習：計算小明今日消耗的卡路里
calorieBurn += activities.ridingBicycle.caloriesPerMinute * 10;
